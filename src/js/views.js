...
export class CardView extends Phaser.GameObjects.Container {
  ...
  _updateState() {
    ...
    var prevCards = this._stackView.model.cards
      .filter((i) => i.order < this._model.order)
    ...
    var targetY =
      this._stackView.y + this._stackView.cardOffset * this._model.order
    ...
    var flightDuration =
      (Phaser.Math.Distance.BetweenPoints(this, { x: targetX, y: targetY }) /
        2000) *
      1000
  ...
  _doubleclick() {
    ...
    var dstStacks = this._model.table.resultStacks.concat(
      this._model.table.workStacks,
    )
  ...
  _dragend() {
    ...
    let nearestStackView = this._stackViews.sort(
      (a, b) =>
        Phaser.Math.Distance.BetweenPoints(a, this) -
        Phaser.Math.Distance.BetweenPoints(b, this),
    )[0]
  ...
  update() {
    ...
    if (
      this._prevCardView != null &&
      (this._prevCardView._dragging || this._prevCardView._following)
    ) {
