...
export class Table extends EventEmitter {
  ...
  switchToNextUnusedCard() {
    if (this._closedUnusedStack.cards.length == 0) {
      ...
      this._openUnusedStack.cards
        .reverse()
        .forEach((c) => this._closedUnusedStack._add(c))
    }
    var closedCards = this._closedUnusedStack.cards
    for (let i = 1; i < 4; i++) {
      this._openUnusedStack._add(closedCards[closedCards.length - i])
    }
    this._incMoves()
  }
  ...
  _addResultsInStorage() {
    const name = prompt()
    const currUserResult = {
      name,
      moves: this._moves,
    }
    ...
    this._showResultsInTable(storedResults)
    storedResults = []
  }
  openTable() {
  ...
  _showResultsInTable(storedResults) {
    const table = document.getElementById('table')
